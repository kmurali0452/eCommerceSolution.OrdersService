...
public class OrdersController : ControllerBase
{
  ...
  public async Task<OrderResponse?> GetOrderByOrderID(Guid orderID)
  {
    FilterDefinition<Order> filter = Builders<Order>.Filter.Eq(temp => temp.OrderID, orderID);
        ...
        if (Environment.GetEnvironmentVariable("MONGODB_HOST") == null)
        {
            ...
            OrderResponse? order= await _ordersService.GetOrderDetails(orderID);
            return order;
  ...
  public async Task<IEnumerable<OrderResponse?>> GetOrdersByProductID(Guid productID)
  {
      ...
      Builders<OrderItem>.Filter.Eq(tempProduct => tempProduct.ProductID, productID)
        ...
        {
            List<OrderResponse?> order = await _ordersService.GetOrderbyDetails(productID);
            return order;
  ...
  public async Task<IEnumerable<OrderResponse?>> GetOrdersByUserID(Guid userID)
  {
        ...
        else
        {
            ...
            List<OrderResponse?> orders = await _ordersService.GetOrderbyUser(userID);
            return orders;
